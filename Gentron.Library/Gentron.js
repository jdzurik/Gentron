...
let Gentron = Gentron_1 = class Gentron {
    ...
    static deserialize(gentron) {
        return _1.Utilities.TaJson.deserialize({
    ...
    static parse(gentronJson) {
        return _1.Utilities.TaJson.parse(gentronJson, Gentron_1);
    }
    static save(iGentron) {
        ...
        const localPackageFolderExists = _1.Utilities.hasStringValue(projectSettings.LocalPackageFolder);
        const packageNameExists = _1.Utilities.hasStringValue(packageSettings.PackageName);
        ...
        const projectSettingsStr = JSON.stringify(_1.Utilities.TaJson.serialize(projectSettings), null, 4);
        ...
        const packageSettingsStr = JSON.stringify(_1.Utilities.TaJson.serialize(packageSettings), null, 4);
    ...
    static open(fileName) {
        ...
        try {
            ret.ProjectSettings = _1.Utilities.TaJson.parse(projectReadResult.Result, _1.ProjectSettings);
        }
        catch (e) {
            return results_1.Result.fail(e.message);
        }
        if (!_1.Utilities.hasStringValue(ret.ProjectSettings.LocalPackageFolder)) {
        ...
        try {
            ret.PackageSettings = _1.Utilities.TaJson.parse(packageReadResult.Result, _1.PackageSettings);
            ret.PackageSettings.DatabaseSources.forEach((source, index) => {
                const connection = ret.ProjectSettings.DatabaseConnections.filter((connection) => {
